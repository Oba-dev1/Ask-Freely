  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-start justify-between mb-10 gap-6 flex-wrap">
        <div>
          <h1 className="text-4xl font-bold text-neutral-900 mb-1.5 tracking-tight">All Events</h1>
          <p className="text-base text-neutral-500">Manage all your Q&A sessions</p>
        </div>
      ...
      {events.length > 0 ? (
            ...
            >
              ...
              <div className={`p-7 ${viewMode === 'list' ? 'flex-1 flex flex-col' : ''}`}>
                <div className="flex items-start justify-between gap-4 mb-4 flex-col md:flex-row">
                  ...
                  <span
                    className={`px-3.5 py-2 rounded-[10px] text-xs font-bold whitespace-nowrap flex items-center gap-1.5 uppercase tracking-wide shadow-[0_2px_4px_rgba(0,0,0,0.05)] ${
                      event.status === 'published'
                        ? 'bg-gradient-to-br from-emerald-500/15 to-emerald-600/10 text-emerald-700 border border-emerald-500/20'
                        : event.status === 'draft'
                        ? 'bg-gradient-to-br from-amber-500/15 to-amber-600/10 text-orange-700 border border-amber-500/20'
                        : 'bg-gradient-to-br from-neutral-500/15 to-neutral-600/10 text-neutral-700 border border-neutral-500/20'
                    }`}
                  >
        ...
        <div className="flex flex-col items-center justify-center py-16 px-8 text-center bg-white rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.06),0_1px_2px_rgba(0,0,0,0.04)] border border-black/[0.04]">
          <i className="fas fa-calendar-plus text-6xl text-neutral-300 mb-5 opacity-60"></i>
          <h3 className="text-xl font-bold text-neutral-900 mb-2 tracking-tight">No Events Yet</h3>
          <p className="text-base text-neutral-500 mb-7 max-w-[400px]">Create your first event to get started</p>
          <button
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-[10px] text-sm font-semibold cursor-pointer transition-all bg-gradient-to-br from-primary to-orange-500 text-white shadow-[0_2px_8px_rgba(255,107,53,0.2)] hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(255,107,53,0.3)]"
            onClick={() => setIsModalOpen(true)}
          >
